...
contract EventTickets {
    ...
    */
    event LogBuyTickets(address purchaser, uint tickets4Purchase);
    event LogGetRefund(address requester, uint tickets4Purchase);
    ...
    */
    function buyTickets(uint tickets4Purchase)
        public
        payable
   {
        require (myEvent.isOpen == true);
        require (msg.value >= TICKET_PRICE * tickets4Purchase);
        require (myEvent.totalTickets - myEvent.sales >= tickets4Purchase);

        myEvent.buyers[msg.sender] += tickets4Purchase;
        myEvent.sales += tickets4Purchase;

        uint refund = msg.value - (TICKET_PRICE * tickets4Purchase);
        msg.sender.transfer(refund);

        emit LogBuyTickets(msg.sender, tickets4Purchase);
   }
